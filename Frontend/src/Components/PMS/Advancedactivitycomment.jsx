...
const AdvancedCommentSystem = ({
  ...
  const generateMockComments = () => {
    ...
    for (let i = 0; i < commentCount; i++) {
      ...
      const comment = {
        ...
        replies: hasReplies
          ? generateMockReplies(`mock-comment-${i}`, user.name)
          : [],
        isPinned: isPinned,
        isPrivate: Math.random() > 0.85,
        attachments: hasAttachments
          ? mockFiles.slice(0, Math.floor(Math.random() * 3) + 1)
          : [],
        editedAt:
          Math.random() > 0.7
            ? new Date(
  ...
  }, [activityId, userId, sortBy, canViewPrivateComments, activityName]);
  ...
  const handleSubmitComment = async (e) => {
    ...
    try {
      ...
      if (editCommentId) {
        ...
        response = await apiService.addComment(commentData, {
  ...
  const stats = useMemo(
    () => ({
      ...
      attachments: comments.reduce(
        (acc, comment) => acc + (comment.attachments?.length || 0),
        0
      ),
    }),
    [comments]
        ...
        <div className="flex items-center justify-between text-sm text-slate-500">
          <div>
            ...
            {stats.private > 0 && canViewPrivateComments && (
              <span>
                {" "}
                • <Lock className="w-3 h-3 inline ml-1" /> {stats.private}{" "}
                private
              </span>
            )}
            {stats.attachments > 0 && (
            ...
            {useMockData && (
              <span>
                {" "}
                • <Zap className="w-3 h-3 inline ml-1" /> Demo Mode
              </span>
            )}
          </div>
          <div className="flex items-center gap-4">
