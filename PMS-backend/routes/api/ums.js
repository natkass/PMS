...
router.route("/finduser/:user_id").get(adminController.getUser);
router
  .route("/findalluser")
  .get(
    verifyJWT,
...
router
  ...
  .post(verifyJWT, adminController.editUserProfile);

router
  .route("/profile/changeStatus/:id")
  .get(
    verifyJWT,
    verifyAccessWithoutProject(process.env.CHANGE_USER_STATUS),
...
router
  .route("/profile/project/:user_id")
  .get(verifyJWT, adminController.getAllUserproject);
